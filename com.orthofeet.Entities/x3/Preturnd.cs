// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;
using Microsoft.EntityFrameworkCore;

#nullable disable

namespace com.orthofeet.Entities
{
    [Table("PRETURND", Schema = "TESTPROD")]
    [Index(nameof(Pnhnum0), nameof(Pndlin0), Name = "PRETURND_PND0", IsUnique = true)]
    [Index(nameof(Pthnum0), nameof(Ptdlin0), Name = "PRETURND_PND1")]
    [Index(nameof(Lininvflg0), nameof(Bpsinv0), nameof(Itmref0), nameof(Pnhnum0), nameof(Pndlin0), Name = "PRETURND_PND2", IsUnique = true)]
    [Index(nameof(Pohnum0), nameof(Poplin0), nameof(Poqseq0), Name = "PRETURND_PND3")]
    public partial class Preturnd
    {
        [Required]
        [Column("PNHNUM_0")]
        [StringLength(20)]
        public string Pnhnum0 { get; set; }
        [Column("PNDLIN_0")]
        public int Pndlin0 { get; set; }
        [Required]
        [Column("CPY_0")]
        [StringLength(5)]
        public string Cpy0 { get; set; }
        [Required]
        [Column("PNHFCY_0")]
        [StringLength(5)]
        public string Pnhfcy0 { get; set; }
        [Column("RTNDAT_0", TypeName = "datetime")]
        public DateTime Rtndat0 { get; set; }
        [Required]
        [Column("POHFCY_0")]
        [StringLength(5)]
        public string Pohfcy0 { get; set; }
        [Required]
        [Column("POHNUM_0")]
        [StringLength(20)]
        public string Pohnum0 { get; set; }
        [Column("POPLIN_0")]
        public int Poplin0 { get; set; }
        [Column("POQSEQ_0")]
        public int Poqseq0 { get; set; }
        [Column("POHTYP_0")]
        public byte Pohtyp0 { get; set; }
        [Required]
        [Column("PTHNUM_0")]
        [StringLength(20)]
        public string Pthnum0 { get; set; }
        [Column("PTDLIN_0")]
        public int Ptdlin0 { get; set; }
        [Required]
        [Column("BPSNDE_0")]
        [StringLength(20)]
        public string Bpsnde0 { get; set; }
        [Required]
        [Column("BPSNUM_0")]
        [StringLength(15)]
        public string Bpsnum0 { get; set; }
        [Required]
        [Column("BPSINV_0")]
        [StringLength(15)]
        public string Bpsinv0 { get; set; }
        [Required]
        [Column("BPAINV_0")]
        [StringLength(7)]
        public string Bpainv0 { get; set; }
        [Required]
        [Column("ITMREF_0")]
        [StringLength(20)]
        public string Itmref0 { get; set; }
        [Required]
        [Column("ITMDES1_0")]
        [StringLength(30)]
        public string Itmdes10 { get; set; }
        [Required]
        [Column("ITMDES_0")]
        [StringLength(30)]
        public string Itmdes0 { get; set; }
        [Column("QTYUOM_0", TypeName = "numeric(28, 13)")]
        public decimal Qtyuom0 { get; set; }
        [Column("QTYPUU_0", TypeName = "numeric(28, 13)")]
        public decimal Qtypuu0 { get; set; }
        [Column("QTYSTU_0", TypeName = "numeric(28, 13)")]
        public decimal Qtystu0 { get; set; }
        [Column("INVQTYPUU_0", TypeName = "numeric(28, 13)")]
        public decimal Invqtypuu0 { get; set; }
        [Column("INVQTYSTU_0", TypeName = "numeric(28, 13)")]
        public decimal Invqtystu0 { get; set; }
        [Column("SRDQTYSTU_0", TypeName = "numeric(28, 13)")]
        public decimal Srdqtystu0 { get; set; }
        [Column("UOMFLG_0")]
        public byte Uomflg0 { get; set; }
        [Column("UOMPUUCOE_0", TypeName = "numeric(18, 7)")]
        public decimal Uompuucoe0 { get; set; }
        [Column("UOMSTUCOE_0", TypeName = "numeric(18, 7)")]
        public decimal Uomstucoe0 { get; set; }
        [Required]
        [Column("UOM_0")]
        [StringLength(3)]
        public string Uom0 { get; set; }
        [Required]
        [Column("PUU_0")]
        [StringLength(3)]
        public string Puu0 { get; set; }
        [Required]
        [Column("STU_0")]
        [StringLength(3)]
        public string Stu0 { get; set; }
        [Required]
        [Column("WEU_0")]
        [StringLength(3)]
        public string Weu0 { get; set; }
        [Required]
        [Column("CPRCUR_0")]
        [StringLength(3)]
        public string Cprcur0 { get; set; }
        [Required]
        [Column("NETCUR_0")]
        [StringLength(3)]
        public string Netcur0 { get; set; }
        [Column("UNTWEI_0", TypeName = "numeric(13, 4)")]
        public decimal Untwei0 { get; set; }
        [Required]
        [Column("RTNREN_0")]
        [StringLength(10)]
        public string Rtnren0 { get; set; }
        [Required]
        [Column("RTNDES_0")]
        [StringLength(30)]
        public string Rtndes0 { get; set; }
        [Column("GROPRI_0", TypeName = "numeric(19, 5)")]
        public decimal Gropri0 { get; set; }
        [Column("PRIREN_0")]
        public short Priren0 { get; set; }
        [Column("DISCRGVAL1_0", TypeName = "numeric(19, 5)")]
        public decimal Discrgval10 { get; set; }
        [Column("DISCRGVAL2_0", TypeName = "numeric(19, 5)")]
        public decimal Discrgval20 { get; set; }
        [Column("DISCRGVAL3_0", TypeName = "numeric(19, 5)")]
        public decimal Discrgval30 { get; set; }
        [Column("DISCRGVAL4_0", TypeName = "numeric(28, 8)")]
        public decimal Discrgval40 { get; set; }
        [Column("DISCRGVAL5_0", TypeName = "numeric(28, 8)")]
        public decimal Discrgval50 { get; set; }
        [Column("DISCRGVAL6_0", TypeName = "numeric(28, 8)")]
        public decimal Discrgval60 { get; set; }
        [Column("DISCRGVAL7_0", TypeName = "numeric(28, 8)")]
        public decimal Discrgval70 { get; set; }
        [Column("DISCRGVAL8_0", TypeName = "numeric(28, 8)")]
        public decimal Discrgval80 { get; set; }
        [Column("DISCRGVAL9_0", TypeName = "numeric(28, 8)")]
        public decimal Discrgval90 { get; set; }
        [Column("DISCRGREN1_0")]
        public short Discrgren10 { get; set; }
        [Column("DISCRGREN2_0")]
        public short Discrgren20 { get; set; }
        [Column("DISCRGREN3_0")]
        public short Discrgren30 { get; set; }
        [Column("DISCRGREN4_0")]
        public short Discrgren40 { get; set; }
        [Column("DISCRGREN5_0")]
        public short Discrgren50 { get; set; }
        [Column("DISCRGREN6_0")]
        public short Discrgren60 { get; set; }
        [Column("DISCRGREN7_0")]
        public short Discrgren70 { get; set; }
        [Column("DISCRGREN8_0")]
        public short Discrgren80 { get; set; }
        [Column("DISCRGREN9_0")]
        public short Discrgren90 { get; set; }
        [Column("NETPRI_0", TypeName = "numeric(19, 5)")]
        public decimal Netpri0 { get; set; }
        [Column("NETPRIPUU_0", TypeName = "numeric(19, 5)")]
        public decimal Netpripuu0 { get; set; }
        [Column("LINAMT_0", TypeName = "numeric(27, 13)")]
        public decimal Linamt0 { get; set; }
        [Column("LINATIAMT_0", TypeName = "numeric(27, 13)")]
        public decimal Linatiamt0 { get; set; }
        [Column("CLCAMT1_0", TypeName = "numeric(27, 13)")]
        public decimal Clcamt10 { get; set; }
        [Column("CLCAMT2_0", TypeName = "numeric(27, 13)")]
        public decimal Clcamt20 { get; set; }
        [Column("CLCAMT3_0", TypeName = "numeric(27, 13)")]
        public decimal Clcamt30 { get; set; }
        [Column("CLCAMT4_0", TypeName = "numeric(28, 8)")]
        public decimal Clcamt40 { get; set; }
        [Column("CLCAMT5_0", TypeName = "numeric(28, 8)")]
        public decimal Clcamt50 { get; set; }
        [Column("CLCAMT6_0", TypeName = "numeric(28, 8)")]
        public decimal Clcamt60 { get; set; }
        [Column("CLCAMT7_0", TypeName = "numeric(28, 8)")]
        public decimal Clcamt70 { get; set; }
        [Column("BASTAXLIN1_0", TypeName = "numeric(27, 13)")]
        public decimal Bastaxlin10 { get; set; }
        [Column("AMTTAXLIN1_0", TypeName = "numeric(27, 13)")]
        public decimal Amttaxlin10 { get; set; }
        [Column("AMTTAXLIN2_0", TypeName = "numeric(27, 13)")]
        public decimal Amttaxlin20 { get; set; }
        [Column("AMTTAXLIN3_0", TypeName = "numeric(27, 13)")]
        public decimal Amttaxlin30 { get; set; }
        [Column("AMTTAXRCP_0", TypeName = "numeric(28, 8)")]
        public decimal Amttaxrcp0 { get; set; }
        [Column("AMTTAXISS_0", TypeName = "numeric(28, 8)")]
        public decimal Amttaxiss0 { get; set; }
        [Column("AMTTAXOTH1_0", TypeName = "numeric(28, 8)")]
        public decimal Amttaxoth10 { get; set; }
        [Column("AMTTAXOTH2_0", TypeName = "numeric(28, 8)")]
        public decimal Amttaxoth20 { get; set; }
        [Column("DEDTAXLIN1_0", TypeName = "numeric(27, 13)")]
        public decimal Dedtaxlin10 { get; set; }
        [Column("DEDTAXLIN2_0", TypeName = "numeric(27, 13)")]
        public decimal Dedtaxlin20 { get; set; }
        [Column("DEDTAXLIN3_0", TypeName = "numeric(27, 13)")]
        public decimal Dedtaxlin30 { get; set; }
        [Column("DEDTAXRCP_0", TypeName = "numeric(28, 8)")]
        public decimal Dedtaxrcp0 { get; set; }
        [Column("DEDTAXISS_0", TypeName = "numeric(28, 8)")]
        public decimal Dedtaxiss0 { get; set; }
        [Column("DEDTAXOTH1_0", TypeName = "numeric(28, 8)")]
        public decimal Dedtaxoth10 { get; set; }
        [Column("DEDTAXOTH2_0", TypeName = "numeric(28, 8)")]
        public decimal Dedtaxoth20 { get; set; }
        [Column("CPRCOE_0", TypeName = "numeric(18, 7)")]
        public decimal Cprcoe0 { get; set; }
        [Column("CPR_0", TypeName = "numeric(19, 5)")]
        public decimal Cpr0 { get; set; }
        [Required]
        [Column("TSICOD_0")]
        [StringLength(20)]
        public string Tsicod0 { get; set; }
        [Required]
        [Column("TSICOD_1")]
        [StringLength(20)]
        public string Tsicod1 { get; set; }
        [Required]
        [Column("TSICOD_2")]
        [StringLength(20)]
        public string Tsicod2 { get; set; }
        [Required]
        [Column("TSICOD_3")]
        [StringLength(20)]
        public string Tsicod3 { get; set; }
        [Required]
        [Column("TSICOD_4")]
        [StringLength(20)]
        public string Tsicod4 { get; set; }
        [Required]
        [Column("PJT_0")]
        [StringLength(20)]
        public string Pjt0 { get; set; }
        [Column("EECINCRAT_0", TypeName = "numeric(28, 8)")]
        public decimal Eecincrat0 { get; set; }
        [Required]
        [Column("ORICRY_0")]
        [StringLength(3)]
        public string Oricry0 { get; set; }
        [Required]
        [Column("BPOCRY_0")]
        [StringLength(3)]
        public string Bpocry0 { get; set; }
        [Column("LINEECFLG_0")]
        public short Lineecflg0 { get; set; }
        [Column("LINPRNFLG_0")]
        public byte Linprnflg0 { get; set; }
        [Column("ORDFLG_0")]
        public byte Ordflg0 { get; set; }
        [Column("LINPSTFLG_0")]
        public byte Linpstflg0 { get; set; }
        [Column("LINPSTDAT_0", TypeName = "datetime")]
        public DateTime Linpstdat0 { get; set; }
        [Column("LININVFLG_0")]
        public byte Lininvflg0 { get; set; }
        [Required]
        [Column("TRSFAM_0")]
        [StringLength(20)]
        public string Trsfam0 { get; set; }
        [Required]
        [Column("VAT_0")]
        [StringLength(5)]
        public string Vat0 { get; set; }
        [Required]
        [Column("VAT_1")]
        [StringLength(5)]
        public string Vat1 { get; set; }
        [Required]
        [Column("VAT_2")]
        [StringLength(5)]
        public string Vat2 { get; set; }
        [Required]
        [Column("TAXRCP_0")]
        [StringLength(1)]
        public string Taxrcp0 { get; set; }
        [Required]
        [Column("TAXISS_0")]
        [StringLength(1)]
        public string Taxiss0 { get; set; }
        [Required]
        [Column("TAXOTH1_0")]
        [StringLength(1)]
        public string Taxoth10 { get; set; }
        [Required]
        [Column("TAXOTH2_0")]
        [StringLength(1)]
        public string Taxoth20 { get; set; }
        [Column("LINPURTYP_0")]
        public byte Linpurtyp0 { get; set; }
        [Required]
        [Column("SATISS_0")]
        [StringLength(1)]
        public string Satiss0 { get; set; }
        [Required]
        [Column("LINTEX_0")]
        [StringLength(12)]
        public string Lintex0 { get; set; }
        [Required]
        [Column("LINSTOFCY_0")]
        [StringLength(5)]
        public string Linstofcy0 { get; set; }
        [Column("STOMGTCOD_0")]
        public byte Stomgtcod0 { get; set; }
        [Required]
        [Column("SRHNUM_0")]
        [StringLength(20)]
        public string Srhnum0 { get; set; }
        [Column("SRDLIN_0")]
        public int Srdlin0 { get; set; }
        [Column("LINAUTFLG_0")]
        public byte Linautflg0 { get; set; }
        [Column("LINTYP_0")]
        public byte Lintyp0 { get; set; }
        [Column("LINCAT_0")]
        public byte Lincat0 { get; set; }
        [Column("LIKQTYCOE_0", TypeName = "numeric(18, 7)")]
        public decimal Likqtycoe0 { get; set; }
        [Required]
        [Column("WRH_0")]
        [StringLength(1)]
        public string Wrh0 { get; set; }
        [Column("XB_MANREC_0")]
        public byte XbManrec0 { get; set; }
        [Required]
        [Column("CREUSR_0")]
        [StringLength(5)]
        public string Creusr0 { get; set; }
        [Column("CREDAT_0", TypeName = "datetime")]
        public DateTime Credat0 { get; set; }
        [Required]
        [Column("UPDUSR_0")]
        [StringLength(5)]
        public string Updusr0 { get; set; }
        [Column("UPDDAT_0", TypeName = "datetime")]
        public DateTime Upddat0 { get; set; }
        [Key]
        [Column("ROWID")]
        public int Rowid { get; set; }
    }
}