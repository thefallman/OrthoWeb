// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;
using Microsoft.EntityFrameworkCore;

#nullable disable

namespace com.orthofeet.Entities
{
    [Table("STOTRKWRK", Schema = "TESTPROD")]
    [Index(nameof(Pronum0), nameof(Lev0), Name = "STOTRKWRK_SKW0", IsUnique = true)]
    [Index(nameof(Creusr0), nameof(Credat0), nameof(Pronum0), nameof(Lev0), Name = "STOTRKWRK_SKW1", IsUnique = true)]
    public partial class Stotrkwrk
    {
        [Required]
        [Column("PRONUM_0")]
        [StringLength(20)]
        public string Pronum0 { get; set; }
        [Required]
        [Column("LEV_0")]
        [StringLength(60)]
        public string Lev0 { get; set; }
        [Column("LEVNUM_0")]
        public short Levnum0 { get; set; }
        [Required]
        [Column("LEVALP_0")]
        [StringLength(20)]
        public string Levalp0 { get; set; }
        [Required]
        [Column("ITMREF_0")]
        [StringLength(20)]
        public string Itmref0 { get; set; }
        [Required]
        [Column("LOT_0")]
        [StringLength(15)]
        public string Lot0 { get; set; }
        [Required]
        [Column("SLO_0")]
        [StringLength(5)]
        public string Slo0 { get; set; }
        [Required]
        [Column("SERNUM_0")]
        [StringLength(20)]
        public string Sernum0 { get; set; }
        [Required]
        [Column("SEL_0")]
        [StringLength(20)]
        public string Sel0 { get; set; }
        [Required]
        [Column("ITMREFSEL_0")]
        [StringLength(20)]
        public string Itmrefsel0 { get; set; }
        [Required]
        [Column("ITMDESSEL_0")]
        [StringLength(30)]
        public string Itmdessel0 { get; set; }
        [Required]
        [Column("LOTSEL_0")]
        [StringLength(15)]
        public string Lotsel0 { get; set; }
        [Column("IPTDAT_0", TypeName = "datetime")]
        public DateTime Iptdat0 { get; set; }
        [Column("MVTSEQ_0")]
        public int Mvtseq0 { get; set; }
        [Required]
        [Column("STOFCY_0")]
        [StringLength(5)]
        public string Stofcy0 { get; set; }
        [Column("VCRTYP_0")]
        public byte Vcrtyp0 { get; set; }
        [Required]
        [Column("VCRNUM_0")]
        [StringLength(20)]
        public string Vcrnum0 { get; set; }
        [Column("VCRLIN_0")]
        public int Vcrlin0 { get; set; }
        [Column("TRSTYP_0")]
        public byte Trstyp0 { get; set; }
        [Required]
        [Column("TRSNUM_0")]
        [StringLength(3)]
        public string Trsnum0 { get; set; }
        [Required]
        [Column("PALNUM_0")]
        [StringLength(10)]
        public string Palnum0 { get; set; }
        [Required]
        [Column("CTRNUM_0")]
        [StringLength(10)]
        public string Ctrnum0 { get; set; }
        [Column("REGFLG_0")]
        public byte Regflg0 { get; set; }
        [Required]
        [Column("VCRNUMORI_0")]
        [StringLength(20)]
        public string Vcrnumori0 { get; set; }
        [Column("VCRTYPORI_0")]
        public byte Vcrtypori0 { get; set; }
        [Column("VCRLINORI_0")]
        public int Vcrlinori0 { get; set; }
        [Required]
        [Column("SLOSEL_0")]
        [StringLength(5)]
        public string Slosel0 { get; set; }
        [Required]
        [Column("SERNUMSEL_0")]
        [StringLength(20)]
        public string Sernumsel0 { get; set; }
        [Column("CREDAT_0", TypeName = "datetime")]
        public DateTime Credat0 { get; set; }
        [Required]
        [Column("CREUSR_0")]
        [StringLength(5)]
        public string Creusr0 { get; set; }
        [Key]
        [Column("ROWID")]
        public int Rowid { get; set; }
    }
}